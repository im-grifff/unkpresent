...
class login extends Controller{
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (!empty($arr_data['status-login'])) {
			...
			if ($role == 'dosen' || $role == 'operator') {
				header('Location: '.APP_PATH.'/home/dosen'); // Redirect to home/dosen page
			} else {
				header('Location: '.APP_PATH.'/home/index'); // Redirect to default home page
			}
	...
	public function student($email_student = ""){
		if(!empty($email_student)){
			$arr_data['classes']= $this->logic("Login_model")->get_student_info($email_student);
			if( !empty($arr_data['classes'])){
				$json_data = json_encode($arr_data['classes']);
				echo $json_data;
			}else{
				echo "KOSONG";
			}
